...
hist(kb07$RTtrunc,freq=FALSE,
     main="Distribution of RTtrunc values")
```

## The details of the linear mixed models

We fit two models, the maximal model, and the final model that was settled on in the accompanying paper [@BatesEtAlParsimonious].
...
We assume that $\epsilon \sim N(0,\sigma^2)$, and that $b_s \sim N(0,\Lambda_s)$, and  
$b_i \sim N(0,\Lambda_i)$; these three terms are assumed to be mutually independent. 
$\Lambda_s$ and $\Lambda_i$ are $8\times 8$ variance-covariance matrices for subject and item respectively; the diagonals have the variance estimates for each predictor, and the off-diagonals have the respective pairwise covariances. For example, a $2\times 2$ variance covariance matrix would have the form
...
Here, $\rho_{12} \, \sigma_{1} \sigma_{2}$ is the Covariance, and the correlation between the two random variables is $\rho_{12}$. Notice that we can construct a $2\times 2$ correlation matrix for the above covariance matrix as shown below. This becomes relevant in the next subsection on prior specificiation.
...
4. The prior on correlation matrices corr$_s$ and corr$L_i$ for subject and item variance-covariance matrices needs some explanation.
...
In Stan, we define priors on $L$, the Cholesky decomposition of the correlation matrix, using 
 the so-called LKJ priors, with parameter $\eta=2$ (see Stan manual for details). Once we have a prior on $L$, we can (a) compute the posterior estimates of the correlation matrix, and (b) generate the adjustments (BLUPs) $b_s$ and $b_i$ shown in the equation above. The procedure for generating 
$b_s$ is: (i) premultiply the diagonalized by-subject variance vector $\tau_s$ with the Cholesky decomposition $L$ to get a matrix $\Lambda_s$, 
(ii) generate random variable $u_s$ from the N(0,1) distribution, and (iii) multiply $\Lambda_s$ with $u_s$ to obtain the correlated random variables. As an illustration, consider the simple case where we have two random variables, which implies two variance components. Let the correlation between the random variables be $-0.6$.  We can generate five pairs of $b_s$ values as follows:
...
## generate 5 pairs of random variables:
z<-rnorm(2*5)
z<-matrix(z,ncol=2)
...
Lambda<-sigmas%*%L%
b_s<-Lambda*%t(z)
## check that the random variables are 
## correlated:
cor(t(b_s))
```
...
Next, we visualize the posterior distributions of the parameters.
